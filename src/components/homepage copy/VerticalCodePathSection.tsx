...
function drawLab(ctx: CanvasRenderingContext2D, w: number, h: number, t: number) {
  ...
  for (let i = 0; i < lanes; i++) {
    ...
    ctx.strokeStyle = `rgba(255, 138, 61, ${0.12 + i * 0.025})`
    ctx.stroke()
    ...
    ctx.fillStyle = `rgba(255, 107, 0, ${0.45 + pulse * 0.35})`
    ctx.fill()
  }
}
...
function drawHtml(ctx: CanvasRenderingContext2D, w: number, h: number, t: number) {
  ...
  for (let i = 0; i < blocks; i++) {
    ...
    ctx.strokeStyle = `rgba(161, 196, 255, ${0.15 + (i % 3) * 0.05})`
    ctx.fillStyle = `rgba(161, 196, 255, ${0.035 + (i % 2) * 0.02})`
    ctx.lineWidth = 1
    ctx.beginPath()
  ...
  ctx.fillStyle = "rgba(161, 196, 255, 0.26)"
...
function drawCss(ctx: CanvasRenderingContext2D, w: number, h: number, t: number) {
  ...
  for (let i = 0; i < cols; i++) {
    ...
    ctx.fillStyle = `rgba(141, 214, 180, ${0.14 + i * 0.012})`
    ctx.fillRect(x, h * 0.76 - height, w * 0.045, height)
  }

  ctx.beginPath()
  ctx.setLineDash([6, 8])
  ctx.strokeStyle = "rgba(141, 214, 180, 0.18)"
  ctx.lineWidth = 1
...
function drawJs(ctx: CanvasRenderingContext2D, w: number, h: number, t: number) {
  ...
  for (let ring = 0; ring < 4; ring++) {
    ...
    ctx.strokeStyle = `rgba(244, 201, 93, ${0.08 + ring * 0.035})`
    ctx.lineWidth = 1
    ctx.stroke()
  }
...
const AnimatedCardShell = memo(function AnimatedCardShell({
  ...
  return (
    <motion.article
      ...
      className="group relative min-h-[560px] overflow-hidden rounded-[16px] border border-white/10 bg-[#0d1015] shadow-[0_30px_70px_-38px_rgba(0,0,0,0.9)]"
    >
      ...
      />
      <div className="pointer-events-none absolute inset-0 border border-white/10 [mask-image:linear-gradient(to_bottom,white,transparent_70%)]" />
      <div className="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-[#0d1015] via-[#0d1015]/92 to-transparent" />
      ...
      <div className="relative z-10 flex h-full min-h-[560px] flex-col justify-between p-5 sm:p-7" style={{ transform: "translateZ(36px)" }}>
        <div className="flex items-start justify-between gap-4">
          <div className="rounded-full border border-white/10 bg-[#11151c]/70 px-4 py-1.5 backdrop-blur-xl">
            <span className={`font-mono text-[9px] font-bold uppercase tracking-[0.28em] ${card.accentClass}`}>
              {card.pill}
            </span>
          </div>
          <div className={`flex h-12 w-12 items-center justify-center rounded-[14px] border border-white/10 bg-[#11151c]/70 backdrop-blur-xl transition-transform duration-500 group-hover:scale-110 ${card.accentClass}`}>
            {card.icon}
          </div>
        </div>

        <div className="mt-auto">
          <span className="mb-4 block font-mono text-[11px] uppercase tracking-[0.24em] text-white/40">
            {card.label}
          </span>
          <h3 className="max-w-[560px] text-2xl font-black leading-tight text-white sm:text-3xl" style={{ fontFamily: "Outfit, sans-serif" }}>
            {card.title}
          </h3>
          <p className="mt-4 max-w-[520px] text-sm leading-7 text-white/56 sm:text-[15px]">
            {card.desc}
          </p>

          <div className="mt-7 overflow-hidden rounded-[12px] border border-white/10 bg-[#07090d]/78 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] backdrop-blur-xl">
            <div className="mb-4 flex items-center gap-2 border-b border-white/10 pb-3">
              <span className="h-2 w-2 rounded-full bg-[#ff6b00]" />
              <span className="h-2 w-2 rounded-full bg-[#a1c4ff]" />
              <span className="h-2 w-2 rounded-full bg-[#8dd6b4]" />
              <span className="ml-2 font-mono text-[10px] uppercase tracking-[0.2em] text-white/35">
                {card.id}.motion
              </span>
            </div>
            <div className="space-y-1.5 font-mono text-[12px] leading-6 text-white/86 sm:text-[13px]">
...
function VerticalRouteGraphic({
  ...
  return (
    <div className="relative h-full min-h-[520px] overflow-hidden rounded-[16px] border border-white/10 bg-[#0d1015]/72 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] backdrop-blur-xl">
      <div className="mb-6 flex items-center gap-3">
        <div className="flex h-9 w-9 items-center justify-center rounded-[12px] border border-white/10 bg-white/[0.04] text-[#ff8a3d]">
          <Route className="h-4 w-4" />
        </div>
        <div>
          <p className="font-mono text-[10px] uppercase tracking-[0.24em] text-white/38">Scroll Route</p>
          <p className="text-sm font-semibold text-white/82">Vertical signal path</p>
        </div>
      </div>
      ...
      <svg aria-hidden="true" viewBox="0 0 220 720" className="h-[calc(100%-72px)] w-full">
        <defs>
          <linearGradient id="softreeVerticalPath" x1="0" x2="0" y1="0" y2="1">
            ...
            <stop offset="38%" stopColor="#a1c4ff" />
            <stop offset="68%" stopColor="#8dd6b4" />
        ...
        {stations.map((station, index) => {
          ...
          return (
            <g key={station.label}>
              <circle cx="110" cy={station.y} r="20" fill="#0d1015" stroke="rgba(255,255,255,0.12)" />
              <motion.circle
                cx="110"
                cy={station.y}
                r="10"
                ...
                fill={isActive ? "#ff8a3d" : "rgba(255,255,255,0.22)"}
              />
              <text
                x="150"
                y={station.y + 4}
                fill={isActive ? "rgba(255,255,255,0.8)" : "rgba(255,255,255,0.28)"}
...
export function VerticalCodePathSection() {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative overflow-hidden bg-[#080a0d] px-4 py-24 text-white sm:px-6 md:py-32"
    >
      <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(255,255,255,0.035)_1px,transparent_1px),linear-gradient(180deg,rgba(255,255,255,0.028)_1px,transparent_1px)] bg-[size:72px_72px]" />
      ...
      <div className="relative mx-auto grid max-w-[1440px] gap-10 lg:grid-cols-[minmax(300px,420px)_minmax(0,1fr)] lg:gap-16">
        <aside className="lg:sticky lg:top-24 lg:h-[calc(100dvh-8rem)]">
          <div className="mb-6">
            <span className="mb-4 inline-flex rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 font-mono text-[10px] uppercase tracking-[0.24em] text-white/56">
              Front-End Build System
            </span>
            <h2 className="max-w-[12ch] text-4xl font-black leading-none text-white sm:text-5xl" style={{ fontFamily: "Outfit, sans-serif" }}>
              Scroll through the code path.
            </h2>
            <p className="mt-5 max-w-[34ch] text-sm leading-7 text-white/55">
...
export default VerticalCodePathSection
