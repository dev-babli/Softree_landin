...
export function SoftreeHeroCarousel() {
  ...
  useGSAP(
    () => {
      ...
      /* gsap-scrolltrigger: scrub: 1 gives smooth 1s catchup (not jerky like true) */
      /* gsap-scrolltrigger: pin:true pins the trigger element */
      const tl = gsap.timeline({
        defaults: { ease: "none" },  // gsap-timeline: defaults for all children
        scrollTrigger: {
          trigger: containerRef.current,
          start: "top top",
          end: "+=420",
          scrub: true, // With Lenis, scrub should be strictly true (1:1), not a number, to avoid double-smoothing jitter.
          pin: true,
        }
      })
  ...
  return (
    <section ref={containerRef} className="relative h-screen bg-[#fafaf9]">
      <div className="sticky top-0 h-screen w-full overflow-hidden flex flex-col items-center">
        ...
        <div className="hero-text-cluster absolute inset-0 z-20 flex flex-col justify-center pl-[12vw] pointer-events-none">
          <div className="max-w-[900px] text-left pointer-events-auto pb-[15vh]">
            <h1
              className="hero-title text-white text-[clamp(48px,5vw,80px)] font-black leading-[0.95] tracking-[-0.03em]"
            ...
            <p
              className="hero-p text-white/70 text-[clamp(15px,1.1vw,18px)] mt-5 font-medium leading-relaxed max-w-[400px]"
              style={{ willChange: "transform, opacity" }}
            >
            ...
            <Link
              href="/services"
              className="hero-btn inline-block mt-7 rounded-full bg-white px-7 py-3.5 text-sm font-bold text-zinc-950 shadow-xl hover:bg-slate-100 transition-colors"
              style={{ willChange: "transform, opacity" }}
            >
              Explore services →
            </Link>
