...
function GlassCard({
  ...
  return (
    ...
    >
      ...
      />
      <div className="absolute inset-0 z-20 pointer-events-none rounded-[16px] border border-[#D1CDC7] [mask-image:linear-gradient(to_bottom,white,transparent)]" />
...
export function LCVirtualOffice() {
  const canvasRefs = [
  ...
  return (
    <section className="relative w-full bg-[#F3F0EE] overflow-hidden">
      <style>{`
        ...
        .lc-vos-glass-pill {
          background: linear-gradient(135deg, rgba(251,100,36,0.35) 0%, rgba(255,255,255,0.05) 100%);
          backdrop-filter: blur(28px) saturate(180%);
          border: 1px solid rgba(209,205,199,0.6);
          border-top-color: rgba(251,100,36,0.5);
          border-left-color: rgba(255,255,255,0.3);
          box-shadow: 0 14px 40px 0 rgba(127,99,21,0.12), inset 0 1px 4px 0 rgba(255,255,255,0.4);
          border-radius: 9999px;
          position: relative;
          overflow: hidden;
        }
        .lc-vos-glass-pill::after {
          content: "";
          position: absolute;
          top: 0; left: -100%;
          width: 50%; height: 100%;
          ...
          animation: lc-vos-glare 5s infinite ease-in-out;
        }
        @keyframes lc-vos-glare { 0% { left: -100%; } 15% { left: 200%; } 100% { left: 200%; } }
      `}</style>

      <div className="pointer-events-none absolute top-0 right-0 w-[700px] h-[500px]"
        style={{ background: "radial-gradient(ellipse at 85% 10%, rgba(251,100,36,0.15) 0%, transparent 70%)", filter: "blur(60px)" }} />
      <div className="pointer-events-none absolute bottom-0 left-0 w-[500px] h-[400px]"
        style={{ background: "radial-gradient(ellipse at 5% 90%, rgba(251,100,36,0.10) 0%, transparent 70%)", filter: "blur(80px)" }} />
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 md:px-8 py-20 md:py-28">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8 mb-14">
          <h2 className="text-4xl md:text-5xl lg:text-[56px] font-black text-[#141413] leading-[1.05] tracking-tight"
            style={{ fontFamily: "Outfit, sans-serif" }}>
            Built for enterprises.{" "}
            <span className="text-[#141413]/30">Delivered globally.</span>
          </h2>
          <p className="text-[#696969] text-base md:text-lg leading-relaxed max-w-[400px] lg:text-right font-medium">
            Softree turns your boldest enterprise ambitions into production-grade software — on time, on spec, every time.
          </p>
        </div>

        <motion.div
          className="grid grid-cols-2 md:grid-cols-4 gap-px bg-[#D1CDC7] rounded-[16px] overflow-hidden mb-12 border border-[#D1CDC7]"
          initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }} transition={{ duration: 0.5 }}>
          {STATS.map((s, i) => (
            <div key={i} className="flex flex-col items-center justify-center py-8 px-4 bg-[#FCFBFA] backdrop-blur-2xl">
              <span className="font-black text-3xl md:text-4xl mb-1 tracking-tight"
                style={{ fontFamily: "Outfit, sans-serif", color: i % 2 === 0 ? "#FF6B00" : "#A1C4FF" }}>
                {s.value}
              </span>
              <span className="text-[10px] font-black tracking-[0.3em] text-[#696969] uppercase text-center">{s.label}</span>
            </div>
          ))}
        </motion.div>
          ...
          {PILLARS.map((p, i) => (
            <GlassCard key={i} glowColor={p.glowColor} className="min-h-[260px] md:min-h-[280px]">
              <div className="absolute inset-0 rounded-[16px] pointer-events-none"
                style={{
                  background: "#FCFBFA",
                  boxShadow: "rgba(127,99,21,0.12) -8px 16px 39px, rgba(127,99,21,0.10) -33px 64px 72px",
                }} />

              <div className="absolute top-0 left-0 w-60 h-60 rounded-full blur-[80px] pointer-events-none opacity-20"
                style={{ background: `radial-gradient(circle, ${p.accentColor} 0%, transparent 70%)` }} />

              <canvas
                ref={canvasRefs[i]}
                className="pointer-events-none absolute inset-0 w-full h-full z-0 opacity-20 mix-blend-multiply group-hover:opacity-40 transition-opacity duration-700"
                style={{ display: "block" }} />

              <div className="relative z-30 p-6 md:p-8 h-full flex flex-col justify-between"
                style={{ transform: "translateZ(20px)" }}>
                <div className="flex items-start justify-between mb-4">
                  <div className="px-4 py-1.5 rounded-full bg-[#FCFBFA]/80 border border-[#D1CDC7] backdrop-blur-2xl">
                    <span className="text-[9px] font-black tracking-[0.3em] uppercase"
                      style={{ color: p.accentColor }}>{p.pill}</span>
                  </div>
                  <span className="text-[9px] font-black tracking-[0.25em] text-[#141413]/20 uppercase"
                    style={{ fontFamily: "Outfit, sans-serif" }}>{p.tag}</span>
                </div>

                <div className="flex-1 flex flex-col justify-end space-y-3">
                  <h3 className="text-xl md:text-2xl font-black text-[#141413] leading-tight tracking-tighter whitespace-pre-line"
                    style={{ fontFamily: "Outfit, sans-serif" }}>{p.title}</h3>
                  <p className="text-[#696969] text-[13px] md:text-sm font-medium leading-relaxed max-w-[340px]">{p.body}</p>
                </div>
              </div>
            </GlassCard>
          ))}
        ...
        <motion.div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mt-10"
          ...
          <Link href="/contact"
            className="lc-vos-glass-pill flex items-center justify-center h-14 px-8 text-[13px] font-black text-white tracking-widest uppercase"
          ...
          <Link href="/services"
            className="flex items-center justify-center h-14 px-8 text-[13px] font-black text-[#141413] hover:text-[#141413]/70 tracking-widest uppercase transition-colors border border-[#D1CDC7] rounded-full backdrop-blur-2xl bg-[#FCFBFA]">
            Our Services
          </Link>
        </motion.div>
...
export default LCVirtualOffice
