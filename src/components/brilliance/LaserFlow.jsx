...
#define TOP_FADE_START 0.95
#define TOP_FADE_EXP 4.0
...
#define HFOG_EDGE_START 0.85
#define HFOG_EDGE_END 0.99
...
#define DITHER_STRENGTH 1.0
    ...
    // Dynamic flare shaping (Sharp Top, Wide/Misty Base)
    float flareY(float y){
        float t = clamp(1.0 - (clamp(y, 0.0, FLARE_HEIGHT) / max(FLARE_HEIGHT, EPS)), 0.0, 1.0);
        // We want the base (low y) to be much wider.
        // As y approaches 0, t approaches 1.
        return pow(t, 0.5); // Using a lower exponent for a wider, more expansive flare at the bottom
    }
...
void mainImage(out vec4 fc,in vec2 frag){
    ...
    for(int k=-TAP_RADIUS;k<=TAP_RADIUS;++k){
        ...
        float m = pow(smoothstep(FLARE_HEIGHT * 1.5, 0.0, yp), 0.8); 
        float wx = 1.0 + (FLARE_AMOUNT * 2.0) * m;
    ...
    float radialFade = 1.0 - smoothstep(0.0, 0.9, length(uvc) / 500.0);
    ...
    // Vertical Gradient Interpolation (Matching Image Atmosphere)
    float cMix = clamp(abs(yPix / (R_V * uVLenFactor * 0.8)), 0.0, 1.0);
    vec3 mixedColor = mix(uColor, uColorSecondary, pow(cMix, 0.8));
...
export const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      uColorSecondary: { value: new THREE.Vector3(0.74, 0.48, 1.0) },
  ...
  useEffect(() => {
    ...
    const sc = hexToRGB(colorSecondary || '#bd7aff');
    if (uniforms.uColorSecondary) {
      uniforms.uColorSecondary.value.set(sc.r, sc.g, sc.b);
    }
  }, [
    wispDensity,
    mouseTiltStrength,
