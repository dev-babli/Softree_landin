  ...
  useGSAP(
    () => {
      ...
      cards.forEach((card, i) => {
        ...
        if (i === 0) {
          gsap.set(els, { autoAlpha: 1, y: 0, x: 0, scale: 1, filter: "blur(0px)" })
        } else {
          gsap.set(els, { autoAlpha: 0, y: 28, x: 14, scale: 0.92, filter: "blur(8px)" })
        }
      })
      ...
      SERVICES.forEach((_, i) => {
        ...
        // ── exit: drift upward, blur in, scale down ──
        ...
        // ── enter: premium reveal — slide from right + blur clear + scale pop ──
