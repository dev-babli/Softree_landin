...
interface SplitFlapTextProps {
  ...
  className?: string
  speed?: number
  /** `light` = light tiles + black glyphs (for light hero backgrounds). Default dark tiles + white glyphs. */
...
function SplitFlapTextInner({ text, className = "", speed = 50, tileTheme = "dark", fontSize }: SplitFlapTextProps) {
...
function SplitFlapChar({
  ...
  const bgColor = isSettled
    ? isLight
      ...
      : "transparent"
    : isLight
      ? "rgba(132, 152, 230, 0.14)"
      : "rgba(132, 152, 230, 0.2)"
  const textColor = isSettled ? (isLight ? "#0a0a0a" : "#ffffff") : "#8498e6"
  ...
  if (isSpace) {
    return (
      <div
        style={{
          width: "0.3em",
          fontSize: fontSize || "clamp(4rem, 15vw, 14rem)",
        }}
      />
    )
  }
  ...
  return (
    ...
    >
      <div
        className={`pointer-events-none absolute inset-x-0 top-1/2 z-10 h-px ${isLight ? "bg-neutral-950/12" : "bg-black/20"}`}
      />
